...
function nextmany(hist,re){
	if(hist[0] == 'IN'){
				prfirst(hist[1])
	}
	else if(inp == 'VB'||inp == 'VBD' ||inp == 'VBG' ||inp == 'VBN' || inp == 'VBP' || inp == 'VBZ' || inp =='V'){

	}
	else{
		for(var i = 0; i < NP.children.length;i++){
			if(hist[0] == NP.children[i].data){
		...
		for(var i = 0; i < NP.children.length;i++){
			if(inp == NP.children[i].data){
				...
				for(var j = 0; j < NP.children[i].children.length;j++){
					if(NP.children[i].children[j].data == '.'){
						for(var k = 0; k < verbtree.children.length;k++){
							re.push(verbtree.children[k]);
						}
					}
					else{
						re.push(NP.children[i].children[j]);
					}
				}
			}
		}
...
function showword(inp){
	...
	if(dat == undefined){
		...
		return dat.slice(Math.floor(Math.random()*dat.length%dat.length),Math.floor(Math.random()*dat.length%dat.length));
...
function similar(inp){
	var dat;
	if(inp == 'JJ' || inp == 'JJR' || inp == 'JJS'){
		dat = 'JJ';
	}
	else if(inp == 'NN' ||inp == 'NNS' || inp == 'NNP' || inp == 'NNPS'){
		dat = 'NN';
	...
	else if(inp == 'RB'||inp == 'WRB' ||inp == 'RBR' || inp == 'RBS'){
		dat= 'RB';
	}//new Node('Interjection');
	else if(inp == 'VB'||inp == 'VBD' ||inp == 'VBG' ||inp == 'VBN' || inp == 'VBP' || inp == 'VBZ' || inp == 'V'){
		dat= 'V';
	}
	else if(inp == 'WDT' || inp == 'DT'){
		dat = 'DT';
	}
	return dat;

}
//console.log(similar('VBN'));
...
function checktaging(inp){
	...
	for (i in taggedWords) {
	    ...
	    if(i == 0){
	    	...
	    	var st = Array.from(new Set(wr));
	    	for(var j = 0; j < st.length;j++){
	    		console.log(st[j]);
	    		printshow(showword(st[j]));
	    	}
	    }
	    else{
	    	var flag = 0;
	    	...
	    	for(var j = 0; j < r.length;j++){	    		
	    		if(r[j].data == tag || r[j].data == similar(tag)){
	    			console.log("found match" + r[j].children.length);
	    			if(r[j].children.length == 0){
	    				var vt = r[j];
	    				//int fl1 = 0;
	    				while(vt.parent){
	    					vt = vt.parent;
	    				...
	    				/*while(vt.parent.data != 'VP' || vt.parent.data != 'NP' || vt.parent.data != 'NPP' ||vt.parent.data != 'AP' || vt.parent.data != 'VPP'){
	    					vt = vt.parent;
	    					fl1 = 1;
	    				}*/
	    			...
	    			flag = 1;
	    			for(var m = 0; m < r[j].children.length;m++){
	    				newr.push(r[j].children[m]);
	    			}
	    			//console.log(newr);
	    		}
	    	}
...
function pathfind(da,path){
	if(da instanceof Node){
		var inp =da.data;
		path.push(da);
		if(inp =='CC' || inp == 'CD'||inp == 'DT'||inp == 'IN'||inp == 'JJ' || inp == 'JJR'|| inp == 'JJS'||inp == 'NN' ||inp == 'NNS' || inp == 'NNP' || inp == 'NNPS'||inp == 'PDT'||inp == 'PRP' || inp == 'PRP$' || inp == 'WP' || inp == 'WP$'||inp == 'RB' ||inp == 'RBR' || inp == 'RBS'||inp == 'TO'||inp == 'UH'||inp == 'VB'||inp == 'VBD' ||inp == 'VBG' ||inp == 'VBN' || inp == 'VBP' || inp == 'VBZ'||inp == 'WDT'||inp == 'WRB'){
			printshow(path);
			return ;
		}
		else{
			if(inp == 'AP' || inp == 'VP' || inp == 'VPP' || inp == 'NP' || inp == 'NPP'){
				pathfind(inp,path);
			}
			else if(inp == 'V'){
				pathfind(verbtree,path);
			}
			var d = da.children.length;
			for(var i = 0; i < d;i++){
				pathfind(da.children[i],path);	
			}
			
		}
	}
}
function printpath(inp){
	var path = [];
	pathfind(inp,path);
...
deter.child('NN','JJ');
deter.children[0].child('.','NN');
deter.children[1].child('NN','JJ');
deter.children[1].children[0].child('.','NN','PRP');
deter.children[1].children[1].child('NN');


var nountree = new Node('NN');
nountree.child('.','CC');
nountree.children[1].child('NN');

var pronountree = new Node('PRP');
pronountree.child('.','NN','JJ');
pronountree.children[1].child('.','NN');
pronountree.children[2].child('NN');

var numbertree = new Node('CD');
numbertree.child('NN');
numbertree.children[0].child('.','NN');
numbertree.children[0].children[1].child('.','CC');
numbertree.children[0].children[1].children[1].child('NN'); 

var adjectivetree = new  Node('JJ');
adjectivetree.child('NN','PRP');
adjectivetree.children[1].child('NN');
